...
int main()
{
	...
	Node * rootNode=(Node*)malloc(sizeof(Node));		//Initialised node
	int i, x[5000],y[5000];
	FILE *fp;
...
void BuildQuadTree(Node *n)
{
	Node * nodeIn =(Node*)malloc(sizeof(Node));	//Initialised node
	...
	if(points > 100)
	{
		for( k =0; k < 4; k++)
		{
			n->child[k] = (Node*)malloc(sizeof(Node));		//Initialised node

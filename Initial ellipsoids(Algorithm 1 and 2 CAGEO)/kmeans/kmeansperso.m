function [idx, C, sumD, D] = kmeansperso(X, k,mass,ray, varargin)
%KMEANS K-means clustering.

%   Matlab Kmeans implementation with few modification of functions distfun and gcentroids to include object balls features
...
%                           for binary data)
%          'madist'      - Percentage of bits that differ (only suitable
%                           for binary data)
%
...
if ischar(distance)
    distNames = {'sqeuclidean','cityblock','cosine','correlation','hamming','madist'};
...
for rep = 1:reps
    ...
    D = distfun(X, C, distance, 0,ray,[],1);
    [d, idx] = min(D, [], 2);
    ...
    try % catch empty cluster errors and move on to next rep
        ...
        nonempties = find(m>0);
        D(:,nonempties) = distfun(X, C(nonempties,:), distance, iter,ray,idx,2);
        d = D((idx-1)*n + (1:n)');
...
while true
    ...
    [C(changed,:), m(changed)] = gcentroids(X, idx, changed, distance,mass);
    D(:,changed) = distfun(X, C(changed,:), distance, iter,ray,idx,2);
    ...
    if ~isempty(empties)
        ...
        case 'singleton'
            ...
            for i = empties
                ...
                m(i) = 1;
                idx(lonely) = i;
                D(:,i) = distfun(X, C(i,:), distance, iter,ray,idx,2);

                % Update clusters from which points are taken
                [C(from,:), m(from)] = gcentroids(X, idx, from, distance,mass);
                D(:,from) = distfun(X, C(from,:), distance, iter,ray,idx,2);
...
function D = distfun(X, C, dist, iter,ray,idx,ind)
...
%              else
%                  disp('autres fois');
%                  %          Recuperer les point du cluster i  
%                  disp(strcat('Cluster numero <',num2str(i),'>'));
%             members = (i == idx);
%             cluster=X(members,:)
%             R=ray(members)
...
%              Distance de tous les points avec le cluster 1
% initialisation
        D(:,i) = (X(:,1) - C(i,1)).^2;
        for j = 2:p
%              Distance de tous les points avec le cluster i
            D(:,i) = D(:,i) + (X(:,j) - C(i,j)).^2;
        end
%           D(:,i)=sqrt(D(:,i))-(ray(i))
...
%     max(P - repmat(R,size(R,2),size(P,2)),[],2)   
%     raycluster=max(P - repmat(R,size(R,2),size(P,2)),[],2)   
    
    D=sqrt(D) - repmat(ray,size(ray,2),size(D,2));
% DD=D
...
function [centroids, counts] = gcentroids(X, index, clusts, dist,mass)
...
%              center = sum(bsxfun(@times, pts, mass), 1)
            ...
            centroids(i,:) = sum(bsxfun(@times,X(members,:),poids),1);
        case 'cityblock'
            % Separate out sorted coords for points in i'th cluster,
